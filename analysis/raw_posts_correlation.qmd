...
```{python}
post_count_ili_sql = get_post_count_ili_sql(
    ili_kws_sql, control_kws_sql,
    lang, country_code
    )
```
...
lags = 2
weeks_ahead = 1
X = post_count_ili_df.drop(['ili_case', 'norm_post_count', 'rest_posts'], axis = 1)
lagdfs = []
...
```{python}
y = post_count_ili_df['ili_case'].iloc[lags+weeks_ahead:]
y = y.divide(y.max())
```
